...
File EspNode::configOpenFile(const char *path, const char *mode)
{
  ...
  { // File exists, reading and loading
    debugPrintln(String(F("SPIFFS: opening file '")) + String(path) + String(F("' mode='")) + String(mode) + String(F("'.")));

    return SPIFFS.open(path, mode);
  }
  else
  {
    debugPrintln(String(F("SPIFFS: file '")) + String(path) + String(F("' mode='")) + String(mode) + String(F("' not found.")));
    return File();
  }
}
